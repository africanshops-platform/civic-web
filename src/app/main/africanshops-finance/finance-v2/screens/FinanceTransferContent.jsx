...
import { useTransfer } from '../hooks/useFintechApi';
          ...
          <AnimatePresence mode="wait">
            {step === STEPS.FORM && (
              <motion.div key="form" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-16">
                ...
                <Button variant="contained" fullWidth
                  disabled={!form.receiverAccountNumber || !form.amount || amountNaira > availableNGN || amountNaira <= 0}
              ...
              <motion.div key="pin" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                <div className="mb-20 rounded-2xl p-16" style={{ background: tokens.pageBg, border: `1px solid ${tokens.borderColor}` }}>
                  <div className="flex justify-between mb-8">
                    ...
                    <Typography style={{ fontSize: F.body, fontWeight: 600, color: tokens.textPrimary, fontFamily: 'monospace' }}>{form.receiverAccountNumber}</Typography>
                  </div>
            ...
            {step === STEPS.OTP && (
              <motion.div key="otp" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                <Typography style={{ fontSize: F.body, color: tokens.textPrimary, marginBottom: 4 }}>An OTP has been sent to your registered email/phone.</Typography>
            ...
            {step === STEPS.SUCCESS && (
              <motion.div key="success" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-16">
                ...
                <Typography style={{ fontSize: F.sectionHead, fontWeight: 800, color: tokens.textPrimary, marginBottom: 8 }}>Transfer Successful!</Typography>
                <Typography style={{ fontSize: F.body, color: tokens.textSecondary, marginBottom: 24 }}>
                  ₦{parseFloat(form.amount || 0).toLocaleString('en-NG', { minimumFractionDigits: 2 })} sent to {form.receiverAccountNumber}
                </Typography>
                {result?.reference && <Typography style={{ fontSize: F.small, color: tokens.textMuted, marginBottom: 16 }}>Ref: {result.reference}</Typography>}
