 ...
 * straight into the registration flow. Behavior is preserved from the old
 * KycBiometricStep fingerprint section — only layout/theme changed.
 */
		...
		<div className="flex flex-col gap-12 mt-8">
			{!webAuthnSupported() ? (
				<Alert
					...
					severity="error"
				>
					WebAuthn is not available in this browser. Use Chrome, Firefox, or Safari on a device with a
				...
				<>
					<Box className="flex flex-col items-center">
						<Box sx={{ position: 'relative', width: 96, height: 96, mb: 2 }}>
							...
							>
								{fpState === 'prompt' ? (
									...
									<FingerprintIcon
										sx={{ fontSize: 34 }}
										color={fpState === 'error' ? 'error' : 'action'}
									/>
								)}
							</Box>
						</Box>
						<Typography
							...
							color={fpState === 'error' ? 'error' : 'text.primary'}
						>
							{fpState === 'prompt'
