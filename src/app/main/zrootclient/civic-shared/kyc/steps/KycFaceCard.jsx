 ...
 * capture + face-api.js detection flow. Behavior is preserved from the old
 * KycBiometricStep face section — only layout/theme changed.
 */
export default function KycFaceCard({ faceVerified = false }) {
	const [localDone, setLocalDone] = useState(faceVerified);
	const [reverifying, setReverifying] = useState(false);
	// Camera/models only ever spin up after an explicit click — never just from
	// this card being on screen. Requested behavior: no live camera feed until
	// the user asks for one, and it's released the moment capture succeeds.
